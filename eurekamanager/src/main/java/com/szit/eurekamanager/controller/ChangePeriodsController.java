...
public class ChangePeriodsController {
     ...
     * @return
     */
    @RequestMapping("/changelists.html")
    protected String listByPage(@RequestParam(value = "cardNum",required = false) String cardNum, @RequestParam(value = "pageIndex",required = false) Integer pageIndex,
        ...
        if(admin!=null){
            ...
            List<CreditCard> lists = changePeriodsService.searchChangePeriodsByPage(cardNum, page);

            Date date =new Date();
            // 保存状态
            model.addAttribute("changeList", lists);
    ...
    @RequestMapping("/changedetails.html")
    protected String changeDetails(@RequestParam("changeDate") String changeDate,
                                    @RequestParam("cardNum") String cardNum, Model model){
        CreditCard card = changePeriodsService.searchChangeDetails(cardNum,changeDate);
     ...
     * @param phone
     */
    @RequestMapping("/changeremiand.html")
    protected void changeRemind(@Param("phone")String phone, HttpServletResponse res){
        ...
        querys.put("content", "【云通知】您的验证码是快还钱。如非本人操作，请忽略本短信");
